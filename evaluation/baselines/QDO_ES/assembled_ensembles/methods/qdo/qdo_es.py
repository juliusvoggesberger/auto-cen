...
from assembled_ensembles.utils.diversity_metrics import get_oracle_predictions, \
...
class QDOEnsembleSelection(AbstractWeightedEnsemble):
    ...
    def ensemble_fit(self, predictions: List[np.ndarray], labels: np.ndarray) -> AbstractWeightedEnsemble:
        ...
        # So as this is NEVER EXPOSED and as such it is IMPOSSIBLE to access the ensemble, the diversity is computed here."
        # I honestly do not understand why this is wrapped in such a way so that a user has absolutelty no ability to get basic information, such as what the base predictors are."
        compute_diversity(predictions, labels)
...
def compute_diversity(predictions, labels):
    ...
    clf_pred = predictions

    diff = [get_oracle_predictions(pred, labels) for pred in clf_pred]
    pairs_idx = list(combinations(range(0, len(predictions)), 2))
