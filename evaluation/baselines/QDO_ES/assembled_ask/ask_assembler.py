...
# Custom Preprocessor for Auto-Sklearn
class OHE(AutoSklearnPreprocessingAlgorithm):
    def __init__(self, **kwargs):
        ...
        categories = [i for i in range(len(self.feat_type)) if list(self.feat_type.values())[i] == 'categorical'
                      or list(self.feat_type.values())[i] == 'string']
        enc = OneHotEncoder(handle_unknown="ignore",sparse=False)
        self.preprocessor = ColumnTransformer(transformers=[("cat", enc, categories)], remainder="passthrough")
...
# Set the OHE Preprocessor in Auto-Sklearn
...
class AskAssembler:
    ...
    def run_ask(self, metric_to_optimize: Scorer, time_limit: int, memory_limit: int,
                max_models_on_disc: Optional[int] = None, n_jobs: int = -1, seed: int = 0,
                n_folds: int = 10):
        ...
        time_limit: int
            Time in seconds for the search
        memory_limit: int
            Memory in MB for the search
        ...
        # Add OHE Preprocessor to Auto-Sklearn (if not already there)
        autosklearn.pipeline.components.data_preprocessing.add_preprocessor(OHE)
        ...
        for iter_idx, (fold_idx, X_train, X_test, y_train, y_test) in enumerate(
            ...
            resampling_strategy = self.get_resampling_strategy(X_train, y_train, n_folds)
            tmp_folder = self.tmp_output_dir.joinpath("fold_{}".format(fold_idx))

            memory = memory_limit
            print("Shape of X_train: " + str(X_train.shape))
            print("Shape of X_test: " + str(X_test.shape))
            print("########## Auto-sklearn parameters: ##########")
            print("- Budget: " + str(time_limit))
            print("- Memory: " + str(memory))
            print("- Max Models on Disc: " + str(max_models_on_disc))
            print("- Number of Jobs: " + str(n_jobs))
            print("- Metric: " + str(metric_to_optimize))
            print("- Intended Strategy Folds: " + str(n_folds))
            print("- Intended Resampling Strategy: " + str(resampling_strategy))
            print("- Intended Resampling Strategy: " + str(self.resampling_strategy))
            print("- Resampling Strategy: " + "cv") # TODO Does this work with the rest of the framework? Else guarantee, that holdout is set with an 80/20 split?
            print("- Folds: " + str(10))
            print("- Cutoff Time Intended: " + str(time_limit))
            print("- Cutoff Time Used: " + str(600))
            print("- Seed: " + str(seed))
            print("- TMP Folder: " + str(tmp_folder))
            print("##############################################")
            # initial_configurations_via_metalearning=0,  # TODO No meta-learning
            automl = classification.AutoSklearnClassifier(time_left_for_this_task=int(time_limit),
                                           memory_limit=memory,
                                           n_jobs=n_jobs,
                                           ...
                                           seed=seed,
                                           per_run_time_limit=600, # Should be 600
                                           include={'feature_preprocessor': ["no_preprocessing"],
                                                    "data_preprocessor": ["OHE"]}
                                           )
            ...
            try:
                ...
                # print(automl.get_models_with_weights()[0][1])
            except:
                print("Cant print auto-sklearn leaderboard")
    ...
    def collect_predictor_data_from_ask_data(self, refit=False, seed=0):
        ...
        for iter_idx, (fold_idx, X_train, X_test, y_train, y_test) in enumerate(
            ...
            bm_dir_names = [os.path.basename(run_folder) for run_folder in
                            glob.glob(str(ask_dir.joinpath("runs/"+str(seed)+"_*")))]
            ...
            # "run_1" somehow changed to "run_0"...
            with open(base_dir.joinpath("smac3-output/run_"+str(seed)+"/runhistory.json"), "r") as f:
            ...
            # -- Store fold errors
            # check if there was an error saved in the fold (or if refit_and_predict_errors value is an empty dict)
            if any(ask_errors_[fold_idx]["refit_and_predict_errors"]):
    ...
    def build_metatask_from_predictor_data(self, pruner=None, metric=None, seed=0):
        ...
        for iter_idx, (fold_idx, ask_ids_to_load) in enumerate(to_load_predictors_per_fold.items(), 1):
            ...
            # "run_1" somehow changed to "run_0"...
            smac_dir = self.tmp_output_dir.joinpath("fold_{}/smac3-output/run_{}/configspace.json".format(fold_idx, seed))
            # Set variable if new
