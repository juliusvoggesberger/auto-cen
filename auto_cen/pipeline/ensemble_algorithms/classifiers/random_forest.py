"""
Implements a wrapper for the sklearn RandomForest implementation.
"""
...
from auto_cen.constants import CLASSIFICATION, MULTICLASS, LABELS, CONTINUOUS_OUT, MULTILABEL, \
    MIXED
...
class RandomForest(BaseMethod):
    """
    Wrapper for the sklearn classifier Random Forest.
    ...
    def __init__(self, n_estimators: int, criterion: str, min_samples_split: int,
                 min_samples_leaf: int, max_features: float, bootstrap: bool, ccp_alpha: float,
                 **kwargs):
    ...
    def get_config_space() -> ConfigurationSpace:
        ...
        ccp_alpha = UniformFloatHyperparameter('ccp_alpha', lower=0.0, upper=1e-3,
                                               default_value=0.0)

        c_space.add_hyperparameters(
            [n_estimators, criterion, min_samples_split, min_samples_leaf, max_features, bootstrap,
             ccp_alpha])
