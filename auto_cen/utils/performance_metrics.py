...
from auto_cen.constants import ACCURACY, MC, BALANCED_ACCURACY, AP_MACRO, AP_MICRO, ROC_AUC_OVO, \
    ROC_AUC_OVR
from auto_cen.pusion.utils import transform_label_vector_to_class_assignment_matrix, \
...
def pr_curve(y_true: np.array, y_pred_prob: np.array, n_classes: int) -> list:
    ...
    if len(y_pred_prob.shape) == 1:
        ...
        y_pred_prob = transform_label_vector_to_class_assignment_matrix(y_pred_prob,
                                                                        n_classes=n_classes)
...
def get_performance_report(y_true: np.array, y_pred: np.array, y_prob: np.array,
                           metrics: Union[str, list], n_classes: int, labels=None) -> dict:
    """
...
def get_cost_report(y_true: np.array, y_pred: np.array, y_prob: np.array, metrics: Union[str, list],
                    n_classes: int, labels=None) -> dict:
    """
    Computes cost.
